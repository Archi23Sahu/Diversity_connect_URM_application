...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $role = $_GET['role'];
          ...
          elseif ($role === "Updateacademiaprofile"){
            ...
            elseif ($role === "Totalcountappliedjobsacademia"){
              $id = $_GET['id'];
              $sql="select JID, COUNT(*) as TotalApplicants from applications where JID IN(Select JID from job_postings where ID='$id')GROUP BY JID;";
              $mysqli =mysqli_query($conn,$sql);
              if ($mysqli) {
                $json_data = array();
                while ($row = mysqli_fetch_assoc($mysqli)) {
