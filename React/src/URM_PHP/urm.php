     ...
     elseif ($role === "Updateurmprofile"){
        ...
        elseif ($role === "Viewappliedjobsurm"){
         ...
         if ($mysqli) {
          ...
          // Query execution failed, handle the error (you can log or display an error message)
          echo json_encode(['phpresult' => []]); // Empty array or some other indication of failure
      }
   }
          //----------------------------view total job urm---------------------------------------
          elseif ($role === "Totalcountappliedjobsurm"){
                            
            $id = $_GET['id'];
          $sql="select JID, count(*) as TotalApplicants from applications where JID IN(Select JID from applications where ID='$id') GROUP BY JID";
