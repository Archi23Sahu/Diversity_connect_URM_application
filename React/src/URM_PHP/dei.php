...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  //----------------------------view job urm---------------------------------------
      ...
      elseif ($role === "DeidashboardfetchJobs"){
        ...
        //-------------------fetch job count for dashboard------------------
        elseif ($role === "Totalcountappliedjobsdei"){
              $id = $_GET['id'];
              $sql="select JID, COUNT(*) as TotalApplicants from applications where JID IN(Select JID from job_postings)GROUP BY JID;";
              $mysqli =mysqli_query($conn,$sql);
              if ($mysqli) {
                $json_data = array();
                while ($row = mysqli_fetch_assoc($mysqli)) {
